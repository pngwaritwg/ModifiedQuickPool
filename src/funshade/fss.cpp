...
// ---------------------------- HELPER FUNCTIONS ---------------------------- //
void myxor(const uint8_t *a, const uint8_t *b, uint8_t *res, size_t s_len){
    size_t i;
    for (i = 0; i < s_len; i++)
    {
        res[i] = a[i] ^ b[i];
...
void DCF_gen_seeded(R_t alpha, uint8_t k0[KEY_LEN], uint8_t k1[KEY_LEN], uint8_t s0[S_LEN], uint8_t s1[S_LEN]){
    ...
    for (i = 0; i < N_BITS; i++)                                         // L4
    {
        ...
        myxor(s0_lose, s1_lose, s_cw, S_LEN);                                     // L10
...
void IC_gen(R_t r_in, R_t r_out, R_t p, R_t q, uint8_t k0_ic[KEY_LEN], uint8_t k1_ic[KEY_LEN]){
    ...
    TO_R_t(&k1_ic[Z_PTR]) = - TO_R_t(&k0_ic[Z_PTR]) + r_out 
                                + (US(p+r_in)  > US(q+r_in))  // alpha_p > alpha_q
                                - (US(p+r_in)  > US(p))       // alpha_p > p
                                + (US(q+r_in+1)> US(q+1))     // alpha_q_prime > q_prime
                                + (US(q+r_in+1)==US(0));      // alpha_q_prime = -1
}
...
R_t IC_eval(bool b, R_t p, R_t q, const uint8_t kb_ic[KEY_LEN], R_t x_hat){
    ...
    R_t output = b*((US(x_hat)>US(p))-(US(x_hat)>US(q+1))) - output_1 + output_2 + TO_R_t(&kb_ic[Z_PTR]);
    return output;
}
